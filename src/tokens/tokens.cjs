// ⚠️  ARCHIVO GENERADO — NO EDITAR A MANO.
// Fuente: tokens/*.json · Generador: build/build-tokens.mjs (Style Dictionary)
// Editá los tokens y corré: npm run tokens:build
const cssVars = {
  "--nl-400": "#a1a1aa",
  "--nl-500": "#71717a",
  "--nl-700": "#3f3f46",
  "--nl-900": "#0f1011",
  "--nl-bg": "#efefef",
  "--nl-text": "#0f1011",
  "--nl-black": "#0f1011",
  "--nl-primary": "#5869f7",
  "--nl-primary-dark": "#2d3bc4",
  "--nl-accent": "#fc624b",
  "--nl-accent-warm": "#ff8c42",
  "--nl-pink": "#f76dee",
  "--nl-magenta": "#b717af",
  "--nl-success": "#42cf8a",
  "--nl-success-dark": "#2ba36a",
  "--nl-success-text": "#053a23",
  "--nl-success-bg": "#eaf9f1",
  "--nl-danger": "#fc624b",
  "--nl-cyan": "#a5f3fc",
  "--nl-blue-24": "#1a1a5e",
  "--nl-blue-28": "#2d1f6e",
  "--nl-white": "#ffffff",
  "--nl-surface": "#dbdcd7",
  "--nl-border-soft": "#dbdcd7",
  "--nl-border-ui": "#c6c7c2",
  "--nl-primary-10": "rgba(88, 105, 247, 0.1)",
  "--nl-primary-20": "rgba(88, 105, 247, 0.2)",
  "--nl-accent-10": "rgba(252, 98, 75, 0.1)",
  "--nl-danger-8": "rgba(252, 98, 75, 0.08)",
  "--fg-1": "var(--nl-text)",
  "--fg-2": "var(--nl-700)",
  "--fg-3": "var(--nl-500)",
  "--fg-4": "var(--nl-400)",
  "--bg-1": "var(--nl-bg)",
  "--bg-2": "var(--nl-white)",
  "--line-soft": "var(--nl-border-soft)",
  "--line-ui": "var(--nl-border-ui)",
  "--nl-font-display": "\"Space Grotesk\", system-ui, sans-serif",
  "--nl-font-body": "Inter, system-ui, sans-serif",
  "--nl-font-mono": "\"JetBrains Mono\", \"SF Mono\", \"Fira Code\", monospace",
  "--nl-tracking-tight": "-0.03em",
  "--nl-tracking-normal": "-0.018em",
  "--nl-tracking-ui": "0.08em",
  "--nl-radius-input": "10px",
  "--nl-radius-card": "20px",
  "--nl-radius-pill": "9999px",
  "--nl-shadow-card": "0 2px 12px rgba(20, 20, 20, 0.08)",
  "--nl-shadow-hover": "0 10px 28px rgba(20, 20, 20, 0.14)",
  "--nl-dur-ui": "220ms",
  "--nl-dur-reveal": "480ms",
  "--nl-ease": "cubic-bezier(0.22, 1, 0.36, 1)",
  "--nl-grad-hero": "linear-gradient(135deg, #5869f7 0%, #b717af 60%, #f76dee 100%)",
  "--nl-grad-primary": "linear-gradient(135deg, #5869f7 0%, #2d3bc4 100%)",
  "--nl-grad-accent": "linear-gradient(135deg, #fc624b 0%, #f76dee 100%)",
  "--nl-grad-mint": "linear-gradient(135deg, #42cf8a 0%, #2ba36a 100%)",
  "--nl-grad-dark": "linear-gradient(180deg, #0f1011 0%, #2d3bc4 100%)",
  "--nl-grad-surface": "linear-gradient(135deg, #ffffff 0%, #dbdcd7 100%)",
  "--nl-grad-brand": "linear-gradient(90deg, #5869f7 0%, #fc624b 100%)",
  "--nl-grad-warm": "linear-gradient(90deg, #fc624b 0%, #ff8c42 100%)",
  "--nl-track-tight": "-0.03em",
  "--nl-text-card": "#2f3038",
  "--nl-accent-dark": "#a83a22",
  "--nl-stat-coral": "var(--nl-accent)",
  "--nl-stat-blue": "var(--nl-primary)",
  "--nl-stat-magenta": "#b717af",
  "--nl-subscribe-bg": "#5869f7",
  "--nl-code-bg": "#0f172a",
  "--nl-success-tint": "#baf3d9",
  "--nl-selected-navy": "#1f2b5c",
  "--nl-grad-text-brand": "var(--nl-grad-brand)"
};

const tokens = {
  "color": {
    "400": "#a1a1aa",
    "500": "#71717a",
    "700": "#3f3f46",
    "900": "#0f1011",
    "bg": "#efefef",
    "text": "#0f1011",
    "black": "#0f1011",
    "primary": "#5869f7",
    "primary-dark": "#2d3bc4",
    "accent": "#fc624b",
    "accent-warm": "#ff8c42",
    "pink": "#f76dee",
    "magenta": "#b717af",
    "success": "#42cf8a",
    "success-dark": "#2ba36a",
    "success-text": "#053a23",
    "success-bg": "#eaf9f1",
    "danger": "#fc624b",
    "cyan": "#a5f3fc",
    "blue-24": "#1a1a5e",
    "blue-28": "#2d1f6e",
    "white": "#ffffff",
    "surface": "#dbdcd7",
    "border-soft": "#dbdcd7",
    "border-ui": "#c6c7c2"
  },
  "opacity": {
    "primary-10": "rgba(88, 105, 247, 0.1)",
    "primary-20": "rgba(88, 105, 247, 0.2)",
    "accent-10": "rgba(252, 98, 75, 0.1)",
    "danger-8": "rgba(252, 98, 75, 0.08)"
  },
  "gradient": {
    "hero": "linear-gradient(135deg, #5869f7 0%, #b717af 60%, #f76dee 100%)",
    "primary": "linear-gradient(135deg, #5869f7 0%, #2d3bc4 100%)",
    "accent": "linear-gradient(135deg, #fc624b 0%, #f76dee 100%)",
    "mint": "linear-gradient(135deg, #42cf8a 0%, #2ba36a 100%)",
    "dark": "linear-gradient(180deg, #0f1011 0%, #2d3bc4 100%)",
    "surface": "linear-gradient(135deg, #ffffff 0%, #dbdcd7 100%)",
    "brand": "linear-gradient(90deg, #5869f7 0%, #fc624b 100%)",
    "warm": "linear-gradient(90deg, #fc624b 0%, #ff8c42 100%)"
  },
  "font": {
    "display": [
      "Space Grotesk",
      "system-ui",
      "sans-serif"
    ],
    "body": [
      "Inter",
      "system-ui",
      "sans-serif"
    ],
    "mono": [
      "JetBrains Mono",
      "SF Mono",
      "Fira Code",
      "monospace"
    ]
  },
  "tracking": {
    "tight": "-0.03em",
    "normal": "-0.018em",
    "ui": "0.08em"
  },
  "radius": {
    "input": "10px",
    "card": "20px",
    "pill": "9999px"
  },
  "shadow": {
    "card": "0 2px 12px rgba(20, 20, 20, 0.08)",
    "hover": "0 10px 28px rgba(20, 20, 20, 0.14)"
  },
  "duration": {
    "ui": "220ms",
    "reveal": "480ms"
  },
  "easing": "cubic-bezier(0.22, 1, 0.36, 1)",
  "alias": {
    "fg-1": "var(--nl-text)",
    "fg-2": "var(--nl-700)",
    "fg-3": "var(--nl-500)",
    "fg-4": "var(--nl-400)",
    "bg-1": "var(--nl-bg)",
    "bg-2": "var(--nl-white)",
    "line-soft": "var(--nl-border-soft)",
    "line-ui": "var(--nl-border-ui)"
  },
  "ext": {
    "track-tight": "-0.03em",
    "text-card": "#2f3038",
    "accent-dark": "#a83a22",
    "stat-coral": "var(--nl-accent)",
    "stat-blue": "var(--nl-primary)",
    "stat-magenta": "#b717af",
    "subscribe-bg": "#5869f7",
    "code-bg": "#0f172a",
    "success-tint": "#baf3d9",
    "selected-navy": "#1f2b5c",
    "grad-text-brand": "var(--nl-grad-brand)"
  }
};

module.exports = { cssVars, tokens };
module.exports.default = tokens;
